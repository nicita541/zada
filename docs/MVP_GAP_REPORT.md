...
| Projects | Done for MVP CRUD | `apps/api/src/routes/projects.ts`, `apps/web/src/App.tsx`, `apps/web/src/store/appStore.ts` | Project list/create/edit/delete/select works with local-first persistence, API sync, project cards, validation, and List/Board tabs. Project List view now uses the same real task rows, filters, and search summary as the core Today flow. Advanced project dashboards remain future work. | High |
| Tasks | Partial | `apps/api/src/routes/projects.ts`, `apps/web/src/store/appStore.ts`, `apps/web/src/App.tsx` | Task create/open/edit/complete/delete works with compact task rows, detail modal, local-first persistence, API CRUD, tags, subtasks, reminders, repeat scheduling, search, filters, core game-dev fields, and board drag ordering. Deeper calendar workflows are still incomplete. | High |
...
| Core UI/UX | Partial | `apps/web/src/App.tsx`, `apps/web/src/styles.css`, `apps/web/src/i18n/*` | Main navigation, Today, Inbox, Projects, Project List/Board, Quick Add, task rows, and Task Detail are now shaped around the "simple entry, powerful depth" workflow. Skeleton modules remain intentionally grouped under Later and still need full product work. | High |
