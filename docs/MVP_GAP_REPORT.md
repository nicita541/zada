...
| Projects | Done for MVP CRUD | `apps/api/src/routes/projects.ts`, `apps/web/src/App.tsx`, `apps/web/src/store/appStore.ts` | Project list/create/edit/delete/select works with local-first persistence, API sync, and List/Board tabs. Advanced project dashboards remain future work. | High |
| Tasks | Partial | `apps/api/src/routes/projects.ts`, `apps/web/src/store/appStore.ts`, `apps/web/src/App.tsx` | Task create/open/edit/complete/delete works with detail modal, local-first persistence, API CRUD, tags, subtasks, reminders, repeat scheduling, search, filters, core game-dev fields, and board drag ordering. Deeper calendar workflows are still incomplete. | High |
| Board / Kanban | Done for MVP board workflow | `apps/web/src/components/board/BoardView.tsx`, `apps/web/src/store/appStore.ts`, `apps/api/src/routes/projects.ts`, `packages/shared/src/board.ts` | Project board has real columns, List/Board tabs, column create/rename/delete/reorder, task add/move/reorder, IndexedDB persistence, API endpoints, and sync queue support. Swimlanes, WIP limits, and advanced board automation are future work. | High |
...
- Project Board uses real board columns from IndexedDB/API, supports drag-and-drop column/task ordering, and queues board column/task move operations for sync.
